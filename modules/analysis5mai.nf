...
process markdown_report {
    ...
    Rscript -e "rmarkdown::render('/home/chbope/Documents/nanopore/clone/nextflow/bin/nextflow_markdown_pipeline3.Rmd',output_file=commandArgs(trailingOnly=TRUE)[21])" "${sample_id}" "${craminoreport}" "${analyse_sample_id_file}" "${nanodx}" "${dictionaire}" "${logo}" "${cnv_plot}" "${tumor_number}" "${annotatecnv}" "${cnv_chr9}" "${cnv_chr7}" "${mgmt_results}" "${merge_results}" "${annotSV_fusion}"  "${terphtml}" "${svannahtml}" "${annotsvhtml}" "${egfr_coverage}" "${idh1_coverage}" "${tertp_coverage}" "\${PWD}/${sample_id}_markdown_pipeline_report.pdf"
