...
class NN_classifier:
    ...
    def predict(self, sample):
        ...
        # Deduplicate probe_ids: PacBio reports both strands separately,
        # producing two rows per CpG. Average them so the left join does not
        # expand beyond len(input_features) and cause a shape mismatch.
        if sample['probe_id'].duplicated().any():
            sample = sample.groupby('probe_id', as_index=False)['methylation_call'].mean()
