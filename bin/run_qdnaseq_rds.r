...
p <- arg_parser("Run a CN analysis via QDNAseq for hg38")
...
p <- add_argument(p, "--cutoff", help="CN cutoff modifier, 0.0-1.0", default='0.5')
...
exportBins(copyNumbersSmooth, bedout, format="bed")
...
# Save the copyNumbersCalled object as an RDS file
rdsout <- paste(argv$out_prefix, "copyNumbersCalled.rds", sep="_")
saveRDS(copyNumbersCalled, file=rdsout)
