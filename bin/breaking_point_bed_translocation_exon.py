    ...
    with open(input_vcf, 'r') as fin, open(output_txt, 'w') as fout:
        for line in fin:
            ...
            if chr_second and pos2:
                # Write start breakpoint (current chromosome/position) - BED format with extra columns for feature tracking
                bed_start = start #- 1
                bed_end = start
                name_start = f"{var_id}|{svtype}|start"
                # Add placeholder columns for feature type, exon number that will be filled by intersectBed
    ...
    with open(input_vcf, 'r') as fin, open(output_txt, 'w') as fout:
        for line in fin:
            ...
            if svlen is not None:
                try:
                    svlen_int = int(svlen)
                    svlen_abs = abs(svlen_int)
                    end_pos = start + svlen_abs
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description="Extract breakpoints from VCF to BED format with exon/intron tracking support")
