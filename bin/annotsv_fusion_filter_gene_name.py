...
columns_to_extract = [1, 2, 3, 4, 5, 6, 8, 10, 11, 13, 14, 15, 16, 17]
...
with open(file1_path, 'r') as file1:
    ...
    try:
        criteria_column_index = headers.index("Gene_name")
    except ValueError:
        raise Exception("The header 'Gene_name' was not found in the file.")
    ...
    for row in reader:
        if len(row) > criteria_column_index:
            ...
            for gene_name in gene_names:
                # Pattern for gene_name/other or other/gene_name
                pattern1 = rf'\b{gene_name};\w+\b'  # gene_name followed by any word
                pattern2 = rf'\b\w+;{gene_name}\b'  # any word followed by gene_name
                
                # Find all matches for both patterns
                matches1 = re.findall(pattern1, criteria_column)
                matches2 = re.findall(pattern2, criteria_column)
                
                # Add all matches to the list
                if matches1:
                    matched_genes.extend(matches1)
                if matches2:
                    matched_genes.extend(matches2)
